...
import tqdm
from openprompt.data_utils.text_classification_dataset import AgnewsProcessor, DBpediaProcessor, ImdbProcessor, AmazonProcessor,AgnewsTitleProcessor,NewstitleProcessor,SnippetsProcessor
import torch
...
from openprompt.prompts import ManualVerbalizer, KnowledgeableVerbalizer, SoftVerbalizer, AutomaticVerbalizer,CptVerbalizer
...
if args.dataset == "agnews":
    ...
    dataset['train'] = AgnewsTitleProcessor().get_train_examples("./datasets/TextClassification/agnews/")
    dataset['test'] = AgnewsTitleProcessor().get_test_examples("./datasets/TextClassification/agnews/")
...
if args.verbalizer == "kpt":
    myverbalizer = KnowledgeableVerbalizer(tokenizer, classes=class_labels, candidate_frac=cutoff, pred_temp=args.pred_temp, max_token_split=args.max_token_split).from_file(path=f"./scripts/{scriptsbase}/knowledgeable_verbalizer.{scriptformat}")
elif args.verbalizer == "cpt":
    myverbalizer = CptVerbalizer(tokenizer, classes=class_labels, candidate_frac=cutoff, pred_temp=args.pred_temp, max_token_split=args.max_token_split).from_file(path=f"./scripts/{scriptsbase}/cpt_verbalizer.{scriptformat}")
